<template>
  <article>
    <div class="flex w-full items-start justify-between gap-4">
      ...
      <div class="w-full">
        <header
          class="mb-2 flex items-center justify-between border-b border-b-gray-300 pb-2 dark:border-b-gray-700"
        >
          <h2 class="flex gap-2 text-lg font-medium">
            <span v-if="nameField === primaryKey">#</span>
            <HighlightedText
              :value="name"
              class="line-clamp-1 text-gray-700 dark:text-gray-300"
            />
          </h2>
        </header>
        <dl class="space-y-2 @xl:space-y-0">
          <div class="@xl:table-row">
            <dt
              class="text-sm font-medium text-gray-800 uppercase @xl:table-cell @xl:p-1 dark:text-gray-200"
              :title="primaryKey as string"
            >
              {{ primaryKey }}
            </dt>
            <dd
              class="table-cell text-justify text-sm font-semibold text-gray-800 @xl:p-1 dark:text-gray-200"
            >
              ...
              <SimilarDocumentsLink
                :document-id="document[primaryKey]"
                class="ml-2"
              />
            </dd>
          </div>
          <template v-for="key of fieldsWithoutPrimaryKey">
            <div class="@xl:table-row">
              <dt
                class="table-cell text-sm font-light text-gray-400 uppercase @xl:p-1 dark:text-gray-500"
                :title="key"
              >
                {{ key }}
              </dt>
...
const picture = computed(() =>
  illustrationAttribute.value
    ...
    : (Object.values(props.document).find(looksLikeAPictureUrl) as
        | string
        | null),
)
