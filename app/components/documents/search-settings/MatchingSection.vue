<template>
  <SettingsSection
    :title="t('title')"
    href="https://www.meilisearch.com/docs/reference/api/search#matching-strategy"
  >
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
      <UFormField
        :label="t('labels.matchingStrategy')"
        :help="t(`help.matchingStrategy.${matchingStrategy}`)"
      >
        <USelect
          v-model="matchingStrategy"
          :items="matchingStrategyItems"
          class="w-full"
        />
      </UFormField>

      <UFormField
        :label="t('labels.rankingScoreThreshold')"
        :help="t('help.rankingScoreThreshold')"
      >
        <UInput
          v-model="rankingScoreThresholdInput"
          type="number"
          :min="0"
          :max="1"
          :step="0.01"
...
const matchingStrategy = defineModel<MatchingStrategies>('matchingStrategy', {
  required: true,
})
const rankingScoreThreshold = defineModel<number | null>(
...
const matchingStrategyItems = computed(() => [
  ...
  {
    label: t('labels.strategies.frequency'),
    value: 'frequency' as MatchingStrategies,
  },
])
...
const rankingScoreThresholdInput = computed<number | ''>({
  get: () =>
    null === rankingScoreThreshold.value ? '' : rankingScoreThreshold.value,
  set: (value) => {
